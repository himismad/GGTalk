...
namespace ESFramework.Boost.Controls
{
    ...
    public partial class FileTransferingViewer : UserControl, IFileTransferingViewer ,IEngineActor
    {
        ...
        void fileTransStarted(TransferingProject info)
        {
            ...
            if (this.InvokeRequired)
            {
                this.Invoke(new CbGeneric<TransferingProject>(this.fileTransStarted), info);
            }
            else
            {
                this.AddFileTransItem(info);
        ...
        void fileTransProgress(string projectID, ulong total, ulong sended)
        {
            ...
            if (this.InvokeRequired)
            {
                this.Invoke(new CbGeneric<string, ulong, ulong>(this.fileTransProgress), projectID, total, sended);
            }
            else
            {
